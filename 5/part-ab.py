...
def go(is_b=False):
    with open("in") as f:
        inp = [list(map(int, line.strip().replace("->", ",").split(","))) for line in f]

    field = [
        [0 for _ in range(max(max([(c[0], c[2]) for c in inp])) + 1)]
        for _ in range(max(max([(c[1], c[3]) for c in inp])) + 1)
    ]
